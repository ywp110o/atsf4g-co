...
#include <type_traits>
static_assert(std::is_pod<db_async_data_t>::value, "db_async_data_t must be a pod, because it will stored in a "
...
int32_t db_msg_dispatcher::unpack_msg(msg_ptr_t msg_container, const void *msg_buf, size_t msg_size) {
    ...
    assert(msg_size == sizeof(db_async_data_t));

    msg_container->mutable_src_server()->set_bus_id(req->bus_id);
    msg_container->mutable_src_server()->set_dst_task_id(req->task_id);
    ...
    case REDIS_REPLY_STATUS: {
        if (0 == UTIL_STRFUNC_STRNCASE_CMP("OK", req->response->str, 2)) {
            ...
            if (req->response->str[10] && req->response->str[11]) {
                msg_container->mutable_dbmsg()->set_version(&req->response->str[11]);
            }
            ret = hello::err::EN_DB_OLD_VERSION;
        } else {
            msg_container->mutable_dbmsg()->set_version(req->response->str);
    ...
    default: {
        if (NULL != req->unpack_fn) {
            ret = req->unpack_fn(*msg_container, req->response);
            if (ret < 0) {
...
const std::string &db_msg_dispatcher::pick_msg_name(const msg_ptr_t msg_container) { return get_empty_string(); }

db_msg_dispatcher::msg_type_t db_msg_dispatcher::pick_msg_type_id(const msg_ptr_t msg_container) { return 0; }

db_msg_dispatcher::msg_type_t db_msg_dispatcher::msg_name_to_type_id(const std::string &msg_name) { return 0; }
...
int db_msg_dispatcher::send_msg(channel_t::type t, const char *ks, size_t kl, uint64_t task_id, uint64_t pd, unpack_fn_t fn, const char *format, ...) {
...
int db_msg_dispatcher::cluster_send_msg(hiredis::happ::cluster &clu, const char *ks, size_t kl, uint64_t task_id, uint64_t pd, unpack_fn_t fn,
    ...
    hiredis::happ::cmd_exec *cmd;
    if (NULL == fn) {
        cmd = clu.exec(ks, kl, NULL, NULL, format, ap);
    } else {
        // 异步数据
        db_async_data_t req;
        req.bus_id = pd;
        ...
        // 防止异步调用转同步调用，预先使用栈上的DBAsyncData
        cmd = clu.exec(ks, kl, cluster_request_callback, &req, format, ap);
