...
router_manager_set::router_manager_set() : save_task_id_(0), last_proc_time_(0) { memset(mgrs_, 0, sizeof(mgrs_)); }
...
int router_manager_set::tick() {
    ...
    do {
        ...
        if (obj->is_writable()) {
            ...
            // 缓存过期
            if (0 == obj->get_cache_task() && 0 == obj->get_object_task() && obj->get_last_visit_time() + cache_expire < last_proc_time_) {
                cache_expired = true;
                save_list_.push_back(auto_save_data_t());
                auto_save_data_t &auto_save = save_list_.back();
                auto_save.object = obj;
    ...
    if (!save_list_.empty() && 0 == save_task_id_) {
        task_manager::me()->create_task_with_timeout<task_action_auto_save_objects>(save_task_id_, logic_config::me()->get_cfg_logic().task_nomsg_timeout,
        ...
        if (0 == save_task_id_) {
            ...
            task_manager::me()->start_task(save_task_id_, start_data);
        }
    }
