...
player::~player() {
    WLOGDEBUG("player %s destroyed", openid_id_.c_str());
}

void player::init(const std::string &openid) {
    openid_id_ = openid;
    data_version_ = 0;
...
uint32_t player::get_player_level() const {
    return player_data_->player_level();
}
...
uint32_t player::get_player_vip_level() const {
    return player_data_->vip_level();
}
...
bool player::is_vip() const {
    return get_player_vip_level() > 0;
}

void player::reset_auto_save() {
    schedule_data_.save_pending_time = 0;
}
