#ifndef _LOGIC_PLAYER_MANAGER_H_
#define _LOGIC_PLAYER_MANAGER_H_
...
public:
    ...
    typedef UTIL_ENV_AUTO_MAP(std::string, player_ptr_t) player_index_t;
...
public:
    player_ptr_t make_default_player(const std::string& openid);
     ...
     * @param user user指针
     */
    void force_erase(const std::string& openid);
     ...
     */
    player_ptr_t load(const std::string &openid, bool force = false);

    size_t size() const;

    player_ptr_t create(const std::string& openid);

    const player_ptr_t find(const std::string& openid) const;
    player_ptr_t find(const std::string& openid);
