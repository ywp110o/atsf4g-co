...
task_action_player_cache_expired::task_action_player_cache_expired(): success_count_(0), failed_count_(0) {}

task_action_player_cache_expired::~task_action_player_cache_expired() {}

int task_action_player_cache_expired::operator()(hello::message_container& msg) {
    ...
    while (!cache_list.empty()) {
        ...
        if (false == cache.save) {
            // 释放用户索引
            player_manager::me()->force_erase(user->get_open_id());
        ...
        if(player_manager::me()->remove(user, true)) {
            ++ success_count_;
            WLOGDEBUG("save player cache %s success", user->get_open_id().c_str());
        } else {
            ++ failed_count_;
            ...
            if (cache.failed_times <= logic_config::me()->get_cfg_logic().player_cache_max_retry_times &&
                ...
                if (NULL != pcache) {
                    ...
                    WLOGWARNING("save player cache %s failed, and will try again later", user->get_open_id().c_str());
                } else {
                    store_to_disk = true;
                    WLOGERROR("retry player user cache %s failed", user->get_open_id().c_str());
                }
            } else {
                store_to_disk = true;
            }
            ...
            if (store_to_disk) {
                // 超出失败次数，强制踢出，缓存用户数据
                WLOGERROR("save player cache %s failed", user->get_open_id().c_str());
                hello::table_user db;
                user->dump(db, true);
