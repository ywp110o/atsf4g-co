...
int task_action_auto_save_players::operator()(hello::message_container& msg) {
    ...
    while (limit_count > 0) {
        ...
        if(false == user->is_removing()) {
            ...
            res = rpc::db::player::set(user->get_open_id().c_str(), user_data, user_version);
            // 如果是序号错误,要能够自动修正
            ...
            if (hello::err::EN_SUCCESS != res) {
                WLOGERROR("auto save user %s failed, ret: %d", user->get_open_id().c_str(), res);
                ++failed_count_;
            } else {
                if (!user_version.empty()) {
                    user->set_version(user_version);
                }
                WLOGDEBUG("auto save user %s success, version = %s", user->get_open_id().c_str(), user_version.c_str());
                ++success_count_;
            }
