...
int main(int argc, char* argv[]) {
    ...
    client->get_option_manager()->bind_cmd("-ip, --host", util::cli::phoenix::assign(client_config::host))
        ->set_help_msg("<domain or ip> set host or ip address of loginsvr");

    client->get_option_manager()->bind_cmd("-p, --port", util::cli::phoenix::assign(client_config::port))
