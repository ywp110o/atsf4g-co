...
namespace script {
    namespace lua {
        ...
        private:
            lua_auto_block(const lua_auto_block &src);
            const lua_auto_block &operator=(const lua_auto_block &src);
        ...
        struct lua_binding_placement_new_and_delete {
            ...
            static int __lua_gc(lua_State *L) {
                ...
                auto obj = static_cast<value_type *>(luaL_checkudata(L, 1, lua_binding_userdata_info<value_type>::get_lua_metatable_name()));
                if (NULL == obj) {
