...
int task_action_login_authorization::operator()(hello::message_container &msg) {
    ...
    do {
        ...
        if (0 != login_data_.login_pd()) {
            ...
            if (ret) {
                WLOGERROR("user %s send msg to 0x%llx fail: %d", login_data_.openid().c_str(), static_cast<unsigned long long>(login_data_.login_pd()), ret);
    ...
    if (hello::err::EN_DB_RECORD_NOT_FOUND == res) {
        ...
        // 注册日志
        WLOGINFO("player %s register account finished, uuid: %s, platform: %u, channel: %u, system: %d", msg_body.open_id().c_str(), uuid.c_str(), plat_id,
...
void task_action_login_authorization::init_login_data(hello::table_login &tb, const ::hello::CSLoginAuthReq &req, const std::string &uuid,
                                                      uint32_t channel_id) {
    ...
    tb.mutable_platform()->mutable_profile()->set_uuid(req.open_id());
