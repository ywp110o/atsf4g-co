...
task_action_player_login::task_action_player_login() : is_new_player_(false) {}
task_action_player_login::~task_action_player_login() {}

int task_action_player_login::operator()(hello::message_container &msg) {
    is_new_player_ = false;

    // 1. 包校验
    msg_cptr_type req = get_cs_request();
    if (NULL == req || !req->has_body() || !req->body().has_mcs_login_req()) {
        if (NULL != req) {
            WLOGERROR("login package error, msg: %s", req->DebugString().c_str());
        } else {
            WLOGERROR("login package error, msg not found");
        }
    ...
    if (user && user->get_login_info().login_code() == msg_body.login_code() &&
        ...
        WLOGDEBUG("player %s relogin using login code", user->get_open_id().c_str());
        ...
        WLOGDEBUG("player %s relogin curr data version:%s", user->get_open_id().c_str(), user->get_version().c_str());
    ...
    if (util::time::time_utility::get_now() > tb.login_code_expired()) {
        WLOGERROR("player %s login code expired", msg_body.open_id().c_str());
    ...
    if (res < 0) {
        WLOGERROR("save login data for %s failed, msg:\n%s", msg_body.open_id().c_str(), tb.DebugString().c_str());
    ...
    if (!user || !user->is_inited()) {
        if (!user) {
            user = player_manager::me()->create(msg_body.open_id());
        }

        hello::table_user tbu;
        // 5. 先尝试从数据库读数据
        res = rpc::db::player::get_all(msg_body.open_id().c_str(), tbu, user->get_version());
        if (hello::err::EN_DB_RECORD_NOT_FOUND != res && res < 0) {
            WLOGERROR("load player data for %s failed, error code:%d", user->get_open_id().c_str(), res);
            set_rsp_code(hello::EN_ERR_SYSTEM);
        ...
        if (hello::err::EN_DB_RECORD_NOT_FOUND == res) {
            ...
            user->get_platform_info().mutable_profile()->set_uuid(uuid);
            ...
            res = rpc::db::player::set(user->get_open_id().c_str(), tbu, user->get_version());
            if (res < 0) {
                WLOGERROR("save player data for %s failed, msg:\n%s", user->get_open_id().c_str(), tbu.DebugString().c_str());
                set_rsp_code(hello::EN_ERR_SYSTEM);
                return res;
            }
    ...
    // TODO login success and try to restore daily limit
    // TODO 尝试重新走未完成的支付订单的流程
    WLOGDEBUG("player %s login curr data version:%s", user->get_open_id().c_str(), user->get_version().c_str());
...
int task_action_player_login::on_success() {
    ...
    player::ptr_t user = player_manager::me()->find(req->body().mcs_login_req().open_id());
    if (!user) {
...
int task_action_player_login::on_failed() {
    ...
    if (NULL != req && req->has_body() && req->body().has_mcs_login_req()) {
        player::ptr_t user = player_manager::me()->find(req->body().mcs_login_req().open_id().c_str());
