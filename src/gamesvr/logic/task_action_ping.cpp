...
int task_action_ping::operator()(hello::message_container& msg) {
    ...
    if (user->get_heartbeat_data().continue_error_times >= logic_config::me()->get_cfg_logic().heartbeat_error_times) {
        ...
        do {
            ...
            if(0 != tb.except().last_except_time() && util::time::time_utility::get_now() - tb.except().last_except_time() <= logic_config::me()->get_cfg_logic().heartbeat_ban_time_bound) {
...
int task_action_ping::on_success() {
    return get_ret_code();
}

int task_action_ping::on_failed() {
    return get_ret_code();
}