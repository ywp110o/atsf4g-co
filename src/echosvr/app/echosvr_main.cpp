...
struct app_handle_on_msg {
	...
	int operator()(atapp::app &app, const atapp::app::msg_head_t *head, const void *buffer, size_t len) {
		...
		case ::atframe::component::service_type::EN_ATST_GATEWAY: {
			...
			case ATFRAME_GW_CMD_POST: {
				...
				int res = app.get_bus_node()->send_data(head->src_bus_id, 0, buffer, len);
				if (res < 0) {
					WLOGERROR("send back post data to 0x%llx failed, res: %d", head->src_bus_id, res);
				}
				break;
			}
			case ATFRAME_GW_CMD_SESSION_ADD: {
				...
				if (0 != req_msg.head.session_id) {
					(*gw_)[req_msg.head.session_id] = head->src_bus_id;
				}
				break;
			}
		...
		default:
			WLOGERROR("receive a message of invalid type:%d", head->type);
			break;
		}
